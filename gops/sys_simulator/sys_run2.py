...
class PolicyRunner:
    ...
    def __init__(
        ...
        self.render_safety_gym = render_safety_gym
        self.args = None
    ...
    def run_an_episode(
        ...
        # env.safe_env.task.random_generator.set_random_seed(seed=test_seed)
        # env.update_world()
        ...
        render_list = []
        # render_list.append(env.render_cam())
    ...
    def draw(self):
        ...
        for i in range(policy_num):
            ...
            cost_list.append(np.array(self.eval_list[i]["cost_list"]))
        ...
        if self.plot_range is None:
            ...
            for i in range(policy_num):
                ...
                cost_list[i] = cost_list[i][start_range:end_range]
        ...
        # Plot cost
        path_cost_fmt = os.path.join(
            self.save_path, "Cost.{}".format(default_cfg["img_fmt"])
        )
        fig, ax = plt.subplots(figsize=cm2inch(*fig_size), dpi=default_cfg["dpi"])

        # save cost data to csv
        cost_data = pd.DataFrame(data=cost_list)
        cost_data.to_csv(os.path.join(self.save_path, "Cost.csv"), encoding="gbk")
        ...
        for i in range(policy_num):
            ...
            sns.lineplot(x=step_list[i], y=cost_list[i], label="{}".format(legend))
        plt.tick_params(labelsize=default_cfg["tick_size"])
        labels = ax.get_xticklabels() + ax.get_yticklabels()
        [label.set_fontname(default_cfg["tick_label_font"]) for label in labels]
        plt.xlabel(x_label, default_cfg["label_font"])
        plt.ylabel("Cost", default_cfg["label_font"])
        plt.legend(loc="best", prop=default_cfg["legend_font"])
        fig.tight_layout(pad=default_cfg["pad"])
        plt.savefig(path_cost_fmt, format=default_cfg["img_fmt"], bbox_inches="tight")
        plt.close()
    ...
    def __load_env(self, test_seed, use_opt: bool = False):
        if use_opt:
        ...
        if self.save_render:
            self.save_path = self.save_path_base + f'/test_seed_{test_seed}'
    ...
    def __run_data(self, env, test_seed):
        for i in range(self.policy_num):
            log_policy_dir = self.log_policy_dir_list[i]
            ...
            eval_dict, tracking_dict = self.run_an_episode(
                test_seed, env, networks, self.init_info, is_opt=False, render=self.render_safety_gym
            )
        ...
        if self.use_opt:
            if self.load_opt_path is not None:
                ...
                eval_dict_opt, tracking_dict_opt = self.run_an_episode(
                    env, opt_controller, self.init_info, is_opt=True, render=self.render_safety_gym
                )
    ...
    def init_env(self, test_seed):
        self.args = self.args_list[0]
        env = self.__load_env(test_seed)
        if hasattr(env, "set_mode"):
            env.set_mode("test")
        return env
        
    def run(self, test_seed):
        env = self.init_env(test_seed)
        self.save_path = self.save_path_base + f'/test_seed_{test_seed}'
        os.makedirs(self.save_path, exist_ok=True)
        self.__run_data(env, test_seed)
