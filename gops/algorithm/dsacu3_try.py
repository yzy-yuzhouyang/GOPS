...
class DSACU3Try(AlgorithmBase):
    ...
    def _compute_loss_q(self, data: DataDict):
        ...
        with torch.no_grad():
            ...
            if self.near_end:
                ...
                q1_next_sample = torch.where(q1_next < q2_next, q1_next_sample, q2_next_sample)
                q2_next_sample = q1_next_sample
            else:
                u_epistamic = ((q1_next - q2_next) / 2) ** 2
                u_aleatoric = (q1_std_next ** 2 + q2_std_next ** 2) / 2
                uncertainty = torch.sqrt(u_epistamic + u_aleatoric * self._get_beta() ** 2)
    ...
    def _update(self, iteration: int, update_beta: bool):
        ...
        if not self.near_end and self._get_beta() < 0.001:
            self.near_end = True
