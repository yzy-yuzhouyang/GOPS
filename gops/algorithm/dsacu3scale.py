...
class DSACU3SCALE(AlgorithmBase):
    ...
    def _compute_gradient(
        ...
        tb_info = {
            ...
            "DSAC2/lambda-RL iter": torch.mean(self.lambda_lower).item() if self.auto_lambda else self.lambda_lower,
    ...
    def _compute_loss_q(self, data: DataDict):
        ...
        with torch.no_grad():
            ...
            # 计算不确定性（基于所有Q网络）
            # 认知不确定性：Q值之间的方差
            q_next_all = torch.stack(q_next_means) # [m, B]
            ...
            uncertainty = torch.sqrt(u_epistemic + factor_aleatoric * u_aleatoric)
            
            q_next_all_mean = torch.mean(q_next_all, dim=0)
            if self.auto_lambda:
                self.lambda_lower = (q_next_all_mean - torch.amin(q_next_all, dim=0)) / \
                                    torch.sqrt(torch.clip(u_epistemic, min=1e-8))
            q_next = q_next_all_mean - self.lambda_lower * uncertainty

            mean_uncertainty = self.lambda_lower * uncertainty
            if self.mean_uncertainty is None:
                self.mean_uncertainty = torch.mean(mean_uncertainty.detach())
            else:
                self.mean_uncertainty = (1 - self.tau_b) * self.mean_uncertainty + \
                                        self.tau_b * torch.mean(mean_uncertainty.detach())
