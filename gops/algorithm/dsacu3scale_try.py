...
class DSACU3SCALETry(AlgorithmBase):
    ...
    def _compute_gradient(
        ...
        if update_beta:
            # 计算平均标准差
            mean_stds_squared = sum(mean_std ** 2 for mean_std in self.mean_stds if mean_std is not None)
            mean_std = (mean_stds_squared / self.networks.num_q) ** 0.5
    ...
    def _compute_loss_q(self, data: DataDict):
        ...
        with torch.no_grad():
            ...
            q_mean = torch.mean(q_next_all, dim=0)
            q_min, _ = torch.min(q_next_all, dim=0)
            factor = (q_mean - q_min) / torch.sqrt(u_epistemic)
            
            # 偶然不确定性：所有Q网络标准差的平均值
            q_next_stds_all = torch.stack(q_next_stds)
            u_aleatoric = torch.mean(q_next_stds_all ** 2, dim=0)
            
            uncertainty = torch.sqrt(u_epistemic + 4 * u_aleatoric * self._get_beta() ** 2)
            
            # 平均Q值
            q_next = q_mean - factor * uncertainty
    ...
    def _compute_loss_policy(self, data: DataDict):
        ...
        q = torch.amin(torch.stack(q_values))
