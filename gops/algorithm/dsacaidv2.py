...
class ApproxContainer(ApprBase):
    ...
    def __init__(self, **kwargs):
        ...
        evidence_args = get_apprfunc_dict("evidence", **kwargs)
        self.evidence: nn.Module = create_apprfunc(**evidence_args)
        self.evidence_target = deepcopy(self.evidence)
        ...
        for i in range(self.num_q):  
            ...
            for p in q_target.parameters():
                p.requires_grad = False
        for p in self.evidence_target.parameters():
            p.requires_grad = False
        ...
        self.evidence_optimizer = Adam(self.evidence.parameters(), lr=kwargs["evidence_learning_rate"])
...
class DSACAIDV2(AlgorithmBase):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.max_evidence = kwargs.get("max_evidence", 1e4)
    ...
    def _compute_gradient(
        ...
        loss_q, ensemble_qs_tensor, ensemble_sigmas_tensor, evidence = self._compute_loss_q(data)
        loss_q.backward()
        ...
        tb_info.update({
            ...
            "DSAC2/critic_mean_evidence-RL iter": evidence.mean().item(),
            "DSAC2/critic_std_evidence-RL iter": evidence.std().item(),
        })
    ...
    def _compute_loss_q(self, data: DataDict):
        ...
        evidence = self.networks.evidence(obs, act)
        for i in range(self.networks.num_q):
            q_name = f"q{i+1}"
            q_network = getattr(self.networks, q_name)
        ...
        evidence_factor = torch.sqrt(1 + 1 / evidence.detach())
        sigmas_tensor *= evidence_factor.unsqueeze(0).expand(
            sigmas_tensor.shape[0], *evidence_factor.shape
        ).squeeze(-1)
        ...
        with torch.no_grad():
            ...
            evidence_next = self.networks.evidence_target(obs2, act2)
            for i in range(self.networks.num_q):
                q_target_name = f"q{i+1}_target"
            ...
            evidence_factor = torch.sqrt(1 + 1 / evidence_next)
            sigma_next_tensor *= evidence_factor.unsqueeze(0).expand(
                sigma_next_tensor.shape[0], *evidence_factor.shape
            ).squeeze(-1)
            u_next_aleatoric = torch.mean(sigma_next_tensor ** 2, dim=0)
            
            factor_aleatoric = (self._get_beta() / self.lambda_lower) ** 2 / evidence_next
        ...
        for i in range(self.networks.num_q):
            ...
            if self.use_huber_loss:
                ...
                effective_variance = sigmas[i].pow(2) * (1 + 1 / evidence.detach())
                q_loss = torch.mean(
                    q_ratio * huber_loss(qs[i], target_q, delta = 50, reduction='none') + \
                    sigma_ratio * huber_loss(effective_variance, residual_value, delta = 50, reduction='none') 
        ...
        max_evidence_tensor = torch.tensor(
            self.max_evidence, 
            dtype=evidence.dtype, 
            device=evidence.device
        )
        evidence_target = torch.min(evidence.detach() + 1, max_evidence_tensor)
        evidential_loss = 0.5 * torch.mean(torch.pow(evidence - evidence_target, 2))
        total_loss += evidential_loss

        return total_loss, q_tensor.mean(dim=1), sigmas_tensor.mean(dim=1), evidence.detach()
    ...
    def _update(self, iteration: int, update_beta: bool):
        for i in range(self.networks.num_q):
            ...
            q_optimizer = getattr(self.networks, q_optimizer_name)
            q_optimizer.step()
        self.networks.evidence_optimizer.step()
        ...
        if iteration % self.delay_update == 0:
            ...
            with torch.no_grad():
                ...
                for p, p_targ in zip(
                    self.networks.evidence.parameters(),
                    self.networks.evidence_target.parameters(),
                ):
                    p_targ.data.mul_(polyak)
                    p_targ.data.add_((1 - polyak) * p.data)
