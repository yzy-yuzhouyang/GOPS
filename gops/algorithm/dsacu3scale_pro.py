...
class ApproxContainer(ApprBase):
    ...
    def __init__(self, **kwargs):
        ...
        # create q networks using setattr
        ...
        # create optimizers using setattr
        for i in range(self.num_q):  
            q_name = f"q{i+1}"
...
class DSACU3SCALEPro(AlgorithmBase):
    ...
    def local_update(
        ...
        # if self._get_beta() < 0.5 * self.beta_init and overestimation > 0:
        if overestimation > 0:
            update_beta = False
    ...
    def _compute_loss_q(self, data: DataDict):
        ...
        with torch.no_grad():
            ...
            if self.share_target:
                ...
                # 使用最接近的q_next_samples作为所有Q网络的标准差学习目标
    ...
    def _compute_loss_behavior_policy(self, data: DataDict):
        ...
        # 平均Q值
        # factor_aleatoric = (self._get_beta() / self.lambda_lower) ** 2
        # q = torch.mean(q_all, dim=0) + \
        #     self.lambda_upper * torch.sqrt(torch.clip(u_epistemic + factor_aleatoric * u_aleatoric, min=1e-8))
        
        # q_mean = torch.mean(q_all, dim=0)
        # sqrt_u_epi = torch.sqrt(torch.clip(u_epistemic, min=1e-8))
        # rel_u_epi = sqrt_u_epi / torch.clip(torch.abs(q_mean), min=1)
        # if self.mean_rel_u_epi is None:
        #     self.mean_rel_u_epi = torch.mean(rel_u_epi.detach())
        # else:
        #     self.mean_rel_u_epi = (1 - self.tau_b) * self.mean_rel_u_epi + \
        #                             self.tau_b * torch.mean(rel_u_epi.detach())
        # factor = rel_u_epi.detach() / self.mean_rel_u_epi * self._get_beta() / self.beta_init
        # q = q_mean + (self.lambda_upper * factor - self.lambda_lower) * sqrt_u_epi

        factor = (self._get_beta() / self.lambda_upper) ** 2
        q = torch.mean(q_all, dim=0) + \
            self.lambda_upper * torch.sqrt(torch.clip(u_epistemic + factor * u_aleatoric, min=1e-8))
