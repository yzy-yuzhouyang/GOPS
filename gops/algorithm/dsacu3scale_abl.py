...
class DSACU3SCALEAbl(AlgorithmBase):
    ...
    def _compute_loss_q(self, data: DataDict):
        ...
        with torch.no_grad():
            ...
            uncertainty = torch.sqrt(u_epistemic)
            
            # 平均Q值
            q_next = torch.mean(q_next_all, dim=0) - 0.5 * uncertainty
    ...
    def _compute_loss_policy(self, data: DataDict):
        ...
        q = torch.mean(torch.stack(q_values), dim=0) - 0.5 * torch.sqrt(u_epistemic)
