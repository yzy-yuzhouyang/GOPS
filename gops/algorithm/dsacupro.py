...
class ApproxContainer(ApprBase):
    ...
    def __init__(self, **kwargs):
        ...
        # create confidence coefficient
        self.beta = nn.Parameter(torch.tensor(kwargs['beta'], dtype=torch.float32))
        ...
        # self.beta_optimizer = SGD([self.beta], lr=kwargs["beta_learning_rate"], momentum=0.5)

    def create_action_distributions(self, logits):
        return self.policy.get_act_dist(logits)


class DSACUPRO(AlgorithmBase):
    ...
    def _compute_gradient(
        ...
        if update_beta:
            ...
            mean_std = ((self.mean_std1 ** 2 + self.mean_std2 ** 2) / 2) ** 0.5
            mean_std = min(mean_std, 0.1)
            overestimation = overestimation * abs(overestimation) / (mean_std ** 2)
    ...
    def _compute_loss_q(self, data: DataDict):
        ...
        with torch.no_grad():
            uncertainty = self._compute_uncertainty(q1, q2, q1_std, q2_std)
        ...
        if self.mean_uncertainty is None:
            self.mean_uncertainty = torch.mean(uncertainty.detach())
        else:
            self.mean_uncertainty = (1 - self.tau_b) * self.mean_std2 + self.tau_b * torch.mean(uncertainty.detach())
        ...
        with torch.no_grad():
            ...
            uncertainty_next = self._compute_uncertainty(q1_next, q2_next, q1_std_next, q2_std_next)
            q_next = (q1_next + q2_next) / 2 - torch.sqrt(uncertainty_next)
        ...
        q1_loss = (self.mean_uncertainty + bias) * torch.mean(
            -(target_q1 - q1).detach() / ( uncertainty + bias) * q1
        )
        - (torch.pow(self.mean_std1, 2) + bias) * torch.mean(
            ((torch.pow(q1.detach() - target_q1_bound, 2)- q1_std_detach.pow(2) ) / 
            (torch.pow(q1_std_detach, 3) + bias)) * q1_std
        )

        q2_loss = (self.mean_uncertainty + bias)*torch.mean(
            -(target_q2 - q2).detach() / ( torch.pow(q2_std_detach, 2) + bias) * q2
        )
        - (torch.pow(self.mean_std2, 2) + bias) * torch.mean(
            ((torch.pow(q2.detach() - target_q2_bound, 2)- q2_std_detach.pow(2) ) / 
             (torch.pow(q2_std_detach, 3) +bias)) * q2_std
        )
    ...
    def _compute_uncertainty(self, q1, q2, std1, std2):
        u_epistamic = ((q1 - q2) / 2) ** 2
        u_aleatoric = (std1 ** 2 + std2 ** 2) / 2
        uncertainty = u_epistamic + u_aleatoric * self._get_beta()
        return uncertainty
