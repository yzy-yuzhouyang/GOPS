...
class DSACU3SCALEPlus(AlgorithmBase):
    ...
    def _compute_loss_q(self, data: DataDict):
        ...
        with torch.no_grad():
            ...
            factor = (self._get_beta() / self.lambda_lower) ** 2
            uncertainty = torch.sqrt(u_epistemic + factor * u_aleatoric)
    ...
    def _compute_loss_policy(self, data: DataDict):
        ...
        for i in range(self.networks.num_q):
            ...
            std_values.append(q_std)
        
        # 使用所有Q网络估计得到的置信下界
        u_epistemic = torch.var(torch.stack(q_values), dim=0)
        # 偶然不确定性：所有Q网络标准差的平均值
        stds_all = torch.stack(std_values)
        u_aleatoric = torch.mean(stds_all ** 2, dim=0)
        
        factor = (self._get_beta() / self.lambda_lower) ** 2
        uncertainty = torch.sqrt(u_epistemic + factor * u_aleatoric)
        q = torch.mean(torch.stack(q_values), dim=0) - \
            self.lambda_lower * torch.sqrt(torch.clamp(uncertainty, min=1e-8))
        
        # factor = self._get_beta() / self.beta_init
        # q = torch.mean(torch.stack(q_values), dim=0) + \
        #     (self.lambda_upper * factor - self.lambda_lower) * torch.sqrt(torch.clamp(uncertainty, min=1e-8))
