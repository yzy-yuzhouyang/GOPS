...
class EvaluatorScale:
    ...
    def run_an_episode(self, iteration, render=True):
        ...
        with torch.no_grad():
            ...
            # 计算Q的置信下界
            u_epistemic = torch.var(q_means_tensor, dim=0)
            if not self.auto_lambda:
                q = torch.mean(q_means_tensor, dim=0) - self.lambda_lower * torch.sqrt(u_epistemic)
            else:
                q = torch.mean(q_means_tensor, dim=0) - torch.mean(self.lambda_lower) * torch.sqrt(u_epistemic)
            Q_output_gpu = q
